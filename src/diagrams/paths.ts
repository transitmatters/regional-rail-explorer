...
const bezierParameterizedPosition = (bezier: Bezier) => (frac: number): Turtle => {
    const { x, y } = bezier.get(frac);
    const { x: dx, y: dy } = bezier.derivative(frac);
    return { x, y, theta: r2d(Math.atan2(dy, dx)) };
};

const path = (strings: TemplateStringsArray, ...args: any[]) => {
...
export const line = (length: number) => (turtle: Turtle): PathSegment => {
...
export const curve = (length: number, angle: number) => (turtle: Turtle): PathSegment => {
...
export const wiggle = (length, width, angle = 0) => (turtle: Turtle): PathSegment => {
    const { x: x1, y: y1, theta } = turtle;
    const nextTheta = theta + angle;
...
export const stationRange = ({ start = null, end = null, stations, commands }): StationRange => {
    return {
        type: "stationRange",
        start,
        end,
